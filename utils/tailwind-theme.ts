import { vars } from "nativewind";

export const themes = {
	light: vars({
		"--color-bg": "#FFFFFF",
		"--color-primary-default": "#A9B5DF",
		"--color-primary": "#A9B5DF",
		"--color-secondary-default": "#2D336B",
		"--color-secondary": "#2D336B",
		"--color-schedule-block": "#F1F1F1",
		"--color-days-text": "#121212",
		"--color-modules-text": "#121212",
		"--color-text-default": "#121212",
		"--text-modal-placeholder": "#121212",
		"--text-modal-focus": "#121212",
		"--text-modal-button": "#FFFFFF",
		"--border-text-input": "#E1E1E1",
		"--title-search-results": "#121212",
		"--sigle-search-results": "#2D336B",
		"--name-search-results": "#121212",
		"--section-search-results": "#4B5563",
		"--teachers-search-results": "#4B5563",
		"--text-course-info": "#121212",
		"--text-about": "#121212",
		"--text-license-button": "#FFFFFF",
		"--text-calendar": "#121212",
		"--next-class-bg": "#2D336B",
		"--next-class-text": "#FFFFFF",
		"--color-cta-text": "#FFFFFF",
		"--next-class-info": "#121212",
		"--text-block-name": "#121212",
	}),
	dark: vars({
		"--color-bg": "#121212",
		"--color-primary-default": "#2D336B",
		"--color-primary": "#2D336B",
		"--color-secondary-default": "#A9B5DF",
		"--color-secondary": "#A9B5DF",
		"--color-schedule-block": "#131313",
		"--color-days-text": "#F1F1F1",
		"--color-modules-text": "#F1F1F1",
		"--color-text-default": "#F1F1F1",
		"--text-modal-placeholder": "#FFFFFF",
		"--text-modal-focus": "#FFFFFF",
		"--text-modal-button": "#2D336B",
		"--border-text-input": "#131313",
		"--title-search-results": "#F1F1F1",
		"--sigle-search-results": "#A9B5DF",
		"--name-search-results": "#F1F1F1",
		"--section-search-results": "#9CA3AF",
		"--teachers-search-results": "#9CA3AF",
		"--text-course-info": "#FFFFFF",
		"--text-about": "#FFFFFF",
		"--text-license-button": "#121212",
		"--text-calendar": "#FFFFFF",
		"--next-class-bg": "#FFFFFF",
		"--next-class-text": "#121212",
		"--color-cta-text": "#2D336B",
		"--next-class-info": "#FFFFFF",
		"--text-block-name": "#FFFFFF",
	}),
};
